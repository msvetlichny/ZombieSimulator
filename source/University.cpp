...
#include <iostream>
    ...
    University::University(int total_population, int X, int Y, int Z, int Q, int P, int R){
        location_id = 2;
        ...
        alarmed_move = P;
        zombie_move = R;
        current_population = (total_population * 0.25); //a percentage of the inputted population
        ...
        if(alarmed_map.size() > 0){ // we have this conditional so that we can potentially skip the for-loop
            for(int i = 0; i < zombie_count; i++){
                if(alarmed_map.size() > 0){
                    ...
                    infection_test(it->first); //it->first is = Person P //POTENTIAL ERROR FLAG HERE
                    tests++;
                }
            }
        }
        ...
        if(ignorant_map.size() > 0){  
            for(int i = 0; i < zombie_count - tests; i++){
                if(ignorant_map.size() > 0){
                    ...
                    infection_test(it->first); //POTENTIAL ERROR FLAG HERE
                    tests++;
                }
            }
